...
public class MeanVariance {
    ...
    public void loadSample(float sample) {
        ...
        if (currentSample == this.sample.length) {
            currentSample = 0;
            // Log.d("MeanVariance", (String.format("MeanVariance: Mean=%+1.4f
            // Uncertainty=%+1.4f (Std Deviation=%+1.4f)", MeanValue, Tolerance,
            // StdDeviation)));
        }
    ...
    public float getMeanValue(int number_of_last_samples) {
        int nsamples = Math.min(sample.length, loaded);
        if (number_of_last_samples <= 0)
            return meanValue;
        if (number_of_last_samples >= nsamples)
            return 0;
        if (nsamples > 0) {
            // ------ Mean value
            double mv = 0;
            ...
            Log.d("MeanVariance", (String.format("MeanVariance: Mean=%+1.4f Uncertainty=%+1.4f (Std Deviation=%+1.4f)",
                    meanValue, tolerance, stdDeviation)));
            return (float) mv;
        }
        return 0;
    }

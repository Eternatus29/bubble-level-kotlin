...
public class CalibrationActivity extends AppCompatActivity implements SensorEventListener {
    ...
    private float calibrationPrecisionIncrement = 0; // The increment of the MIN_CALIBRATION_PRECISION in case of
                                                     // calibration reset
    ...
    ToneGenerator toneGen1 = new ToneGenerator(AudioManager.STREAM_MUSIC,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (preferences.contains(KEY_PREF_CALIBRATION_ANGLE_0)) {
            textViewLastCalibration.setText(getString(R.string.calibration_active_calibration) + "\n"
                    ...
                    + String.format(" = %1.3f; %1.3f; %1.3f", preferences.getFloat(KEY_PREF_CALIBRATION_GAIN_0, 0),
                            preferences.getFloat(KEY_PREF_CALIBRATION_GAIN_1, 0),
                            preferences.getFloat(KEY_PREF_CALIBRATION_GAIN_2, 0))
                    + "\n"
                    + getString(R.string.calibration_active_calibration_offsets)
                    + String.format(" = %1.3f; %1.3f; %1.3f", preferences.getFloat(KEY_PREF_CALIBRATION_OFFSET_0, 0),
                            preferences.getFloat(KEY_PREF_CALIBRATION_OFFSET_1, 0),
                            preferences.getFloat(KEY_PREF_CALIBRATION_OFFSET_2, 0))
                    + "\n"
                    + getString(R.string.calibration_active_calibration_angles)
                    + String.format(" = %1.2f°; %1.2f°; %1.2f°", preferences.getFloat(KEY_PREF_CALIBRATION_ANGLE_2, 0),
                            preferences.getFloat(KEY_PREF_CALIBRATION_ANGLE_1, 0),
                            preferences.getFloat(KEY_PREF_CALIBRATION_ANGLE_0, 0)));
        }
    ...
    protected void onResume() {
        ...
        if (PreferenceManager.getDefaultSharedPreferences(this).getBoolean(KEY_PREF_KEEP_SCREEN_ON, true))
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        else
            getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);

        if (currentStep < STEP_COMPLETED) {
            // If the app was calibrating when the onPause is called, the app goes back 1
            // step (the previous screen)
    ...
    private void startStep() {
        ...
        switch (currentStep) {
            ...
            case STEP_COMPLETED:
                ...
                for (int i = 0; i < 7; i++) {
                    Log.d("Clinometer", String.format("mean[ ][" + i + "]  =  %+1.4f  %+1.4f  %+1.4f", mean[0][i],
                            mean[1][i], mean[2][i]));
                }

                // Calibration offset and Gain
                // (https://www.digikey.it/it/articles/using-an-accelerometer-for-inclination-sensing)
                ...
                // CalibrationOffset[2] = (Mean[2][0] + Mean[2][0]) / 2 - (CalibrationGain[2] *
                // STANDARD_GRAVITY);

                Log.d("Clinometer", "-- ACCELEROMETERS ----------------------------------------------------------");
                Log.d("Clinometer", String.format("Offset  =  %+1.4f  %+1.4f  %+1.4f", calibrationOffset[0],
                        calibrationOffset[1], calibrationOffset[2]));
                Log.d("Clinometer", String.format("Gain    =  %+1.4f  %+1.4f  %+1.4f", calibrationGain[0],
                        calibrationGain[1], calibrationGain[2]));
                ...
                for (int i = 0; i < 7; i++) {
                    ...
                    angle[2][i] = (float) (Math.toDegrees(Math.asin(mean[2][i]
                            / Math.sqrt(mean[0][i] * mean[0][i] + mean[1][i] * mean[1][i] + mean[2][i] * mean[2][i]))));
                    Log.d("Clinometer", String.format("angle[ ][" + i + "] =  %+1.4f°  %+1.4f°  %+1.4f°", angle[0][i],
                            angle[1][i], angle[2][i]));
                }
                ...
                Log.d("Clinometer", String.format("Cal.Angles =  %+1.4f°  %+1.4f°  %+1.4f°", calibrationAngle[2],
                        calibrationAngle[1], calibrationAngle[0]));
                Log.d("Clinometer", "----------------------------------------------------------------------------");
                ...
                textViewLastCalibration.setText(getString(R.string.calibration_active_calibration) + "\n"
                        ...
                        + String.format(" = %1.3f; %1.3f; %1.3f",
                                preferences.getFloat(KEY_PREF_CALIBRATION_OFFSET_0, 0),
                                preferences.getFloat(KEY_PREF_CALIBRATION_OFFSET_1, 0),
                                preferences.getFloat(KEY_PREF_CALIBRATION_OFFSET_2, 0))
                        + "\n"
                        + getString(R.string.calibration_active_calibration_angles)
                        + String.format(" = %1.2f°; %1.2f°; %1.2f°",
                                preferences.getFloat(KEY_PREF_CALIBRATION_ANGLE_2, 0),
                                preferences.getFloat(KEY_PREF_CALIBRATION_ANGLE_1, 0),
                                preferences.getFloat(KEY_PREF_CALIBRATION_ANGLE_0, 0)));
                textViewLastCalibration.setVisibility(View.VISIBLE);
                buttonNext.setVisibility(View.VISIBLE);
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            if ((int) (currentStep / 2) * 2 == currentStep) {
                ...
                int progress2 = (int) (Math.min(1000, Math.max(0, 1000 - 1000
                        * (mvGravity0.getTolerance() / (MIN_CALIBRATION_PRECISION + calibrationPrecisionIncrement)))));
                progressBar.setSecondaryProgress(Math.max(progress1, progress2));
                progressBar.setProgress(Math.min(progress1, progress2));

                // DEVICE MOVED

                // Log.d("Clinometer",String.format("[#] Mean value = %+1.5f Reading = %+1.5f
                // Difference = %+1.5f", mvGravity0.getMeanValue(), event.values[0],
                // mvGravity0.getMeanValue() - event.values[0]));

                // if (mvGravity0.isReady() && (mvGravity0.getTolerance() >
                // MIN_CALIBRATION_PRECISION)) {
                if (mvGravity0.isReady() && ((Math.abs(mvGravity0.getMeanValue()
                        - event.values[0]) > (MIN_CALIBRATION_PRECISION + calibrationPrecisionIncrement)) ||
                        (Math.abs(mvGravity1.getMeanValue() - event.values[1]) > (MIN_CALIBRATION_PRECISION
                                + calibrationPrecisionIncrement))
                        ||
                    ...
                    mvGravity1.reset();
                    mvGravity2.reset();
                    if (calibrationPrecisionIncrement < 0.15f)
                        calibrationPrecisionIncrement += 0.01f;
                    // Log.d("Clinometer",String.format("[#] Mean value = %+1.5f Reading = %+1.5f
                    // Difference = %+1.5f", mvGravity0.getMeanValue(), event.values[0],
                    // mvGravity0.getMeanValue() - event.values[0]));
                    Log.d("Clinometer", String.format("[#] New calibration precision = %+1.5f",
                            MIN_CALIBRATION_PRECISION + calibrationPrecisionIncrement));
